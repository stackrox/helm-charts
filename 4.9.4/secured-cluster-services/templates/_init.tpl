...
{{ $_ := include "srox.admissionControllerDefaulting" (list $ .Release $._rox.admissionControl $.stackroxHelm) }}
...
{{ range $idx, $elem := $additionalCAList }}
  ...
  {{/* In a k8s secret, no characters other than alphanumeric, '.', '_' and '-' are allowed. Also, for the
       import-additional-cas script to work, the file names must end in '.crt'. */}}
