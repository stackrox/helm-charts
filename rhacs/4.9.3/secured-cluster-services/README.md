...
|`image.main.registry`| Address of the registry you are using for main image.|`registry.redhat.io/advanced-cluster-security` |
|`image.collector.registry`| Address of the registry you are using for collector image.|`registry.redhat.io/advanced-cluster-security` |
...
|`admissionControl.enforce`| This setting controls whether the admission controller enforces security policies that have been configured for enforcement in StackRox Kubernetes Security Platform. |`true` |
|`admissionControl.failurePolicy`| This setting controls the failure policy for the ValidatingWebhookConfiguration. Can be "Ignore" or "Fail". |"Ignore" |
