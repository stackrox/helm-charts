...
{{/* Only merge scanner config shapes if feature flag is enabled and kubectl output is disabled */}}
{{ $configShapeScanner := $.Files.Get "internal/scanner-config-shape.yaml" | fromYaml }}
  {{ $_ := include "srox.mergeInto" (list $rox $configShape $configShapeScanner) }}
...
{{ $state := dict "notes" list "warnings" list "referencedImages" dict "generated" dict }}
...
{{ if kindIs "invalid" $._rox.env.openshift }}
  {{ $_ := set $._rox.env "openshift" (has "apps.openshift.io/v1" $._rox._apiServer.apiResources) }}
{{ end }}
...
{{/* Disable scanner always in kubectl outputs */}}


{{/* Disable local scanner on non-OpenShift clusters by default. */}}
{{ if not ._rox.env.openshift }}
  {{ if eq $._rox.scanner.disable false }}
    {{ include "srox.fail" (printf "Scanner was enabled but is only supported on OpenShift. Set scanner.disable=true to continue." ) }}
  {{ end }}
...
{{ if and ._rox.env.openshift (eq ._rox.scanner.disable false) }}
